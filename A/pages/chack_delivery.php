        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right" style="float:left!important;">
                    ...
                    <li class="breadcrumb-item active">توصيل الطلبات</li>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
...
$sel_or_no_ch_mo = mysqli_query($con, "SELECT DISTINCT `id_order`,`cu_name`,`cus_phone`,`type_order`,`address_order` FROM `order`JOIN order_item ON id_order=order_item.fk_order WHERE `chack_order`=1 AND  `status_order`=1   AND  `chack_money`=1 AND `processor_order`=1 AND `delivery_order`=0 AND `com_id_ord`=" . $_SESSION['comid'] . " ORDER BY `order`.`date_add` ASC;") or die(mysqli_error($con));
if (mysqli_num_rows($sel_or_no_ch_mo) == 0) {
    echo "<h6 class='text-center'> لا يوجد طلبات للتوصيل</h6>";
} else {


    while ($da_se_ch_mo = mysqli_fetch_array($sel_or_no_ch_mo)) {

        $total_pris_coun = mysqli_query($con, "SELECT COUNT(id_item_order)cou,SUM(total_item)price_total FROM `order_item` WHERE `fk_order` =" . $da_se_ch_mo['id_order'] . "  AND `com_id_item_ord`=" . $_SESSION['comid'] . "   ;") or die(mysqli_error($con));
...
<div class="card collapsed-card">
    ...
    <div class="card-header border-transparent">
        <div class="row">
            ...
            <div class="col-5">
                <form action="insert_data.php" method="post" type="form" name="form_check_procesee" style="float:left;"
                    enctype="multipart/form-data">
                    <label s id="po_deli" class="btn btn-primary-sm">
                        <i class="fa fa-upload"></i>

                        <input type="file" class="form-control" name="picture" id="img_chek_delivery"
                            accept=".png, .jpg,.gif,.jpeg,jpe,.ico" style="display:none;">
                        اختر الصورة
                    </label>
                    <script>
                    $("#po_deli").click(function() {
                        $("#img_chek_delivery").click();
                    });
                    </script>
                    <button type="submit" class="btn btn-success btn-sm " id="id_order_check_deliyery"
                        name="id_order_check_deliyery" value="<?php echo $da_se_ch_mo['id_order'] ?>" s><i
...
<div class="card collapsed-card">
    ...
    <div class="card-body " style="display: none;">
        ...
        $result = mysqli_query($con, "SELECT DISTINCT `id_order`,`cu_name`,`cus_phone`,`type_order`,`address_order` FROM `order`JOIN order_item ON id_order=order_item.fk_order WHERE `chack_order`=1 AND  `status_order`=2   AND  `chack_money`=1 AND `processor_order`=1 AND `delivery_order`=1 AND `com_id_ord`=" . $_SESSION['comid'] . " ORDER BY `order`.`date_add` ASC;") or die(mysqli_error($con));
        if (mysqli_num_rows($result) == 0) {
            echo "<h6 class='text-center'>لا يوجد طلبات موصلة</h6>";
        } else {
                    ...
                    <table class="table m-0">
                        <thead>
                            <tr>
                                ...
                                <th style="padding: 0.30rem 0.0 rem;">
                                    إجمالي
                                </th>
