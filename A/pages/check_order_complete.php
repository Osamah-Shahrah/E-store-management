                ...
                <div class="col-sm-12 table-responsive">
                    <table id="check_order_compang" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            $result = mysqli_query($con, "SELECT DISTINCT `id_order`,`cu_name`,`cus_phone`,`type_order`,`address_order` FROM `order`JOIN order_item ON id_order=order_item.fk_order WHERE `chack_order`=1 AND  `status_order`=2   AND  `chack_money`=1 AND  `delivery_order`=1 AND `processor_order`=1 AND `com_id_ord`=" . $_SESSION['comid'] . " ORDER BY `order`.`date_add` ASC;") or die(mysqli_error($con));
